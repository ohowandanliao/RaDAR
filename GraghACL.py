...
class AclModel(nn.Module):
    ...
    def __init__(self, hid_dim, temp, num_MLP=1, G= None):
        ...
        # self.dgl_graph = dgl.from_networkx(G)
    ...
    def forward(self, v, u, graph):
        pos_score, graph = self.pos_score(graph, v, u)
        neg_score = self.neg_score(v, graph)
        loss = (- pos_score + neg_score).mean()

        return loss
    # def neg_score(self, h, graph, rff_dim=None, batch_size=64):
    #     z = F.normalize(h, dim=-1)
    #     node_count = z.size(0)
    #     neg_score = torch.zeros(node_count).to(z.device)
    #     for start in range(0, node_count, batch_size):
    #         end = min(start + batch_size, node_count)
    #         z_batch = z[start:end]  # current batch chunk
    #         neg_sim_batch = torch.zeros(z_batch.size(0), node_count).to(z.device)
        
    #         for inner_start in range(0, node_count, 32):
    #             inner_end = min(inner_start + 32, node_count)
    #             z_inner_batch = z[inner_start:inner_end]
    #             neg_sim_chunk = torch.exp(torch.mm(z_batch, z_inner_batch.t()) / self.temp)
    #             neg_sim_batch[:, inner_start:inner_end] = neg_sim_chunk
        
    #         neg_score_batch = neg_sim_batch.sum(1)
    #         neg_score[start:end] = neg_score_batch
